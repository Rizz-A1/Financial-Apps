...
fun AddKasbonScreen(
    ...
    val title = if (kasbonId != null && kasbonId != 0L) {
        stringResource(R.string.edit_kasbon)
    } else {
        stringResource(R.string.add_kasbon)
    }
    
    LaunchedEffect(kasbonId) {
        if (kasbonId != null && kasbonId != 0L) {
            viewModel.loadKasbon(kasbonId)
        } else {
            viewModel.resetForm()
        }
    }
